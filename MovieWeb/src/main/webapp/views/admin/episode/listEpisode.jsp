...
<body>
    ...
    <div class="container mt-5" id="list-episode">
...
$(document).ready(function() {
	console.log('đã chạy')
	pagination(3,1,null,'${movieId}');
});

function handleSearchClick(size,page,movieId) {
    var search = $('#search-input').val()
    console.log(search)
    pagination(size,1,search,movieId)
} 

function handleSizeChange(value,search,movieId) {
    // Gọi hàm pagination với giá trị mới được chọn
    pagination(value,1,search,movieId)
}


function pagination(size,page,search,movieId) {
	var size =size;
	var page = page;
	var search = search
	var movieId =  movieId
	 $.get('/MovieWeb/admin/episode/pagination', {
	    	size:size,
	    	page:page,
	    	search:search,
	    	movieId:movieId
	    }).done( function (data) {
	    	$('#list-episode').html(data)
	    
	    	
	    });
}

function btnDelete(movieId,episodeId) {
	var movieId =movieId;
	var episodeId = episodeId;

	 $.get('/MovieWeb/admin/episode/delete', {
	    	movieId:movieId,
	    	episodeId:episodeId
	    }).done( function (data) {
	    	 ...
	    	 Swal.fire({
	        	  ...
	        	  if (result.isConfirmed) {
	        			pagination(3,1,null,data.movieId);
	        	  }
	        	});
